...
public final class FRY_Spawner
{
    ...
    protected final int entityDespawnCheck(World worldObj, EntityLiving entityliving)
    {
        ...
        if (entityliving instanceof FRY_EntityFairy && !((FRY_EntityFairy)entityliving).canDespawn())
        {
    ...
    public final int countEntities(Class class1, World worldObj)
    {
        ...
        for (int j = 0; j < worldObj.loadedEntityList.size(); j++)
        {
            ...
            {
                FRY_EntityFairy fairy = (FRY_EntityFairy)entity;

                if (!fairy.isDead && fairy.health > 0 && fairy.tamed())
                {
