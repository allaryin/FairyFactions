...
public class FRY_EntityFairy extends EntityAnimal
{
    ...
    public void handlePosted(boolean flag)   //The AI method which handles post-related activities.
    {
        ...
        if (!flag)  //Processes fishing, then returns, if sitting.
        {
            if (fishEntity != null)
            {
                ...
                {
                    mod_FairyMod.setPrivateValueBoth(EntityLiving.class, this, "currentTarget", "r", fishEntity);
        ...
        if (!worldObj.isRemote && (ridingEntity == null || ridingEntity == player || ridingEntity instanceof EntityMinecart))
        {
            ...
            if (tamed() && player.username.equals(rulerName()))
            {
                ...
                else
                {
                    ...
                    {
                        mod_FairyMod.fairyMod.sendFairyMount(this, player);
                        setFlymode(true);
                        flyTime = 200;
                        setCanFlap(true);
                        return true;
    ...
    public void disband()
    {
        ...
        if (ruler != null)
        {
            ...
            if (ruler instanceof EntityPlayerMP)
            {
    ...
    public void tameMe(EntityPlayer player)
    {
        ...
        EntityPlayerMP prevRuler = (EntityPlayerMP)ruler;

        if (ruler != null && ruler instanceof EntityPlayerMP)
        {
            prevRuler = (EntityPlayerMP)ruler;
        }

        String prevName = rulerName();
        ...
        if (ruler != null)
        {
            if (ruler instanceof EntityPlayerMP)
            {
                EntityPlayerMP newRuler = (EntityPlayerMP)ruler;
                ...
                if (!prevName.equals("") && i < 2)
                {
                    s += "thought you were a bit nicer than " + prevName + ".";
                }
                else if (!prevName.equals("") && i > 3)
                {
                    s += "left " + prevName + " behind for you.";
                }
                else if (!prevName.equals(""))
                {
                    s += "is surprisingly unfaithful to " + prevName + ".";
                }
                else if (queen() && i < 2)
                {
                ...
                if (prevRuler != null)
                {
                    ...
                    if (j == 0)
                    {
                        t += "Someone bribed " + f + " to join his/her team.";
                    }
                    else if (j == 1)
                    {
                        t += "Someone found out that " + f + " is very negotiable.";
                    }
                    else if (j == 2)
                    {
                        t += f + " left you in favor someone else";
                    }
                    else
                    {
                        t += f + " was made an offer she couldn't resist.";
                    }

                    mod_FairyMod.fairyMod.sendDisband(prevRuler, "* §9" + t);
                    //ModLoader.getMinecraftInstance().ingameGUI.addChatMessage("* §9" + t);
                }
            }
        }
    }
    ...
    public void castRod()
    {
        ...
        else
        {
            ...
            entityFear = null;
            //Server only
            mod_FairyMod.fairyMod.sendFairyMount(hook, this);
        }
    }
    ...
    @Override public boolean attackEntityFrom(DamageSource damagesource, int i)
    {
        ...
        if (flag && health > 0)
        {
            ...
            {
                mod_FairyMod.fairyMod.sendFairyMount(this, ridingEntity);
            }
        }
        else if (flag)
        {
    ...
    @Override protected void attackEntity(Entity entity, float f)
    {
        if (attackTime <= 0 && f < (tamed() ? 2.5F : 2.0F) && ((entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY < boundingBox.maxY) || f == 0F))
        {
            ...
            {
                mod_FairyMod.fairyMod.sendFairyMount(this, entity); //Scout's Totally Leet Air Attack.
                setFlymode(true);
                flyTime = 200;
                setCanFlap(true);
                attackTime = 35;
            ...
            else
            {
                ...
                {
                    mod_FairyMod.fairyMod.sendFairyMount(this, entity); //The finish of its air attack.
                    attackTime = 35;
                }
    ...
    @Override public double getYOffset()
    {
        if (ridingEntity != null)
        {
            //if(ridingEntity instanceof EntityPlayerSP) {  No such thing as EntityPlayerSP in the server files.
    ...
    public String getDisplayName()
    {
        ...
        else
        {
            if (tamed())
            {
                return getActualName(getFairyName1(), getFairyName2());
            }
            else
            {
                return (String)null;
    ...
    @Override public String getUsername()   //Detailed Kill Messages
    {
        String s = "";

        if (getFaction() <= 0)
        {
            return "a " + (tamed() ? "tamed " : "wandering ") + (queen() ? "former " : "") + "fairy " +
                    (queen() ? "queen " : guard() ? "guard " : scout() ? "scout " : medic() ? "medic " : rogue() ? "rogue " : "") + (tamed() ? "named " +
                            getActualName(getFairyName1(), getFairyName2()) : "");
        }
        else
        {
            if (queen())
            {
                return "Queen " + getActualName(getFairyName1(), getFairyName2()) + " of the " + getFactionName(getFaction());
            }
            else
            {
                return "a fairy " + (guard() ? "guard " : scout() ? "scout " : medic() ? "medic " : rogue() ? "rogue " : "") +
                        "from the " + getFactionName(getFaction());
            }
        }
    }
